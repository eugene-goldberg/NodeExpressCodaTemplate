...
export class TCSliderComponent
  ...
  @HostBinding('class.tc-slider') true;
	...
	@HostBinding('style.width') get getSliderWidth() { return this.direction == 'vertical' ? this.width + 'px' : null };
  ...
  public dotOffsetLeft(dot: number) {
    return this.direction == 'vertical' ? 0 : this.toPercentage(dot);
  }

  public dotOffsetTop(dot: number) {
    return this.direction != 'vertical' || this.toPercentage(dot);
  }

  public getTrackHeight(): number {
    return this.direction == 'vertical' ? this.toPercentage(this.value) : null;
  }

  public getTrackOffset() {
    if (this.range) {
			return this.direction == 'vertical' ? 0 : this.toPercentage(this.value[0]);
    } else {
      return 0;
    }
	}
	public getTrackTopOffset() {
		if (this.range) {
			return this.direction != 'vertical' ? null : this.toPercentage(this.value[0]);
		} else {
			return null;
		}
	}
  ...
  public moveHandler(x: number) {
    ...
    if (this.direction == 'vertical') {
      ...
      mousePosition = ((x - this.leftOffset) * 100) / this.elementWidth;
    }
  ...
  offsetTop(value: number): number {
    return this.direction == 'vertical' ? this.toPercentage(value) : null;
  }
  offsetLeft(value: number): number {
    return this.direction != 'vertical' ? this.toPercentage(value) : 0;
  }
